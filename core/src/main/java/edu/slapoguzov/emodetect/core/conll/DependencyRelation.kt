...
enum class DependencyRelation(val text: String) {
    ...
    companion object {
        fun parse(value: String): DependencyRelation {
            return DependencyRelation.values().find { it.text == value }
                    ?: DependencyRelation.values().find { value.contains(it.text) }
                    ?: OTHER
        }
    }
}
